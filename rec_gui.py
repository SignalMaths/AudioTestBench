...
class RecGui(tk.Tk):
    ...
    def __init__(self):
        ...
        self.w = MenuWindow.SettingsWindow(self, 'Settings')
        self.Info ='NONE'
    ...
    def generation_settings(self, *args):
        self.w = MenuWindow.Generate(self, 'Generate')

    def on_rec(self):
        self.recording = True
        filename = 'Sim_'+datetime.now().strftime('%Y_%m_%d_%H_%M_%S')+'.wav'
        self.manageStream.create_stream(self.w.input_dev['index'],filename)
    ...
    def on_stop(self, *args):
        ...
        self.manageStream.stop_stream(self.w.input_dev['index'])
        self.init_buttons()
    ...
    def update_gui(self):
        self.Info_label['text'] = self.w.Info
        #try:
